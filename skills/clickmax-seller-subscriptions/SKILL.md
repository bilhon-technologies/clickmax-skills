...
- update-card is safe/idempotent for the same card choice
...
3. For destructive or broad changes, make the requested action explicit before mutating.

## Execute guide

- Use `mcp__clickmax__subscriptions_get` with `id = subscription id` to inspect one subscription.
- Use `mcp__clickmax__subscriptions_list` with `page`, `perPage`, and the needed filters such as `searchText`, `subscriptionStatus`, `productIds`, `column`, and `order` to find one customer or a cohort.
- Use `mcp__clickmax__subscriptions_chart` with `transactionStatus` and `membershipPeriod.from/to` for counts and trend views inside a billing or membership window.
- Use `mcp__clickmax__subscriptions_update_card` with `id` and `cardId` to swap the billing card for one subscription.
- Use `mcp__clickmax__subscriptions_cancel` with `id`, `cancellationReason`, and optional `cancellationReasonDescription` to cancel one subscription.
- Use `mcp__clickmax__subscriptions_mass_manage` with `productId`, `operation`, and optional `leadIds` only for product-scoped bulk cancel or renew actions.
